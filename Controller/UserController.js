...
exports.friend_request = async (req, res) => {
    if (req.user.username !== req.params.username) {
        try {
            ...
            const receiver = await UserModel.findOne({ username: req.params.username })

            if (!receiver.request.includes(req.user._id) && !receiver.friends.includes(req.user._id)) {
                ...
                if (!sender.followings.includes(receiver._id) && !receiver.followers.includes(sender._id)) {
                    await sender.updateOne({
...
exports.unfriend = async (req, res) => {
    try {
        ...
        if (friend.friends.includes(me._id) && me.friends.includes(friend._id)) {
            ...
            await me.updateOne({
                $pull: { followers: friend._id, followings: friend._id, friends: friend._id }
            })
...
exports.requested_friend = async (req, res) => {
    ...
    try {
        ...
        }).sort({ createdAt: -1 }).select('username -_id first_name last_name picture')
        res.status(200).send(getAll)
    } catch (error) {
        res.status(500).send({ message: error.message })
    }
...
exports.getAllUser = async (req, res) => {
    try {
        const all = await UserModel.find({
            _id: {
                $nin: [req.user._id, ...req.user.requestTo, ...req.user.friends, ...req.user.request]
            }
        }).select('username first_name last_name picture')
...
exports.getPhotos = async (req, res) => {
    ...
    Cloudinary.search
        .expression(`resource_type:image AND folder:facebook/user/${req.user._id}${folder}/*`)
        .sort_by('created_at', 'desc')
        .max_results(max)
        .execute().then(result => res.status(200).send(result.resources))
...
exports.editBio = async (req, res) => {
    ...
    const { currentCity, homeTown, family, highSchool, collage, work, savePost } = req.body
    if (currentCity || homeTown || family || highSchool || collage || work || savePost) {
        return res.send({ message: "You can update only bio" })
    }
    try {
        const update = await UserModel.findOneAndUpdate({ _id: req.user._id }, {
            details: {
                ...req.user.details,
                bio: req.body.bio
            }
...
exports.editCover = async (req, res) => {
    ...
    if (!req.body.cover) {
        ...
        try {
            Cloudinary.uploader.upload(req.body.cover, {
                unique_filename: true,
...
exports.removeCover = async (req, res) => {
    try {
        await UserModel.updateOne({ _id: req.user._id }, {
            $set: { cover: "https://res.cloudinary.com/kajolroy/image/upload/v1653506943/facebook/default_image/fb_cover_pr3fez.png" }
        }, { new: true }).exec()
        res.status(400).send({ message: error.message })
    } catch (error) {
        res.status(500).send({ message: error.message })
...
exports.searchUserAdd = async (req, res) => {
    ...
    try {
        ...
        if (user == req.user._id) return res.status(200).send({ message: "You dot't add your self !!" })
...
exports.visitProfile = async (req, res) => {
    try {
        const findUser = await UserModel.findOne({ username: req.params.username }).populate({
            path: "friends",
            model: UserModel,
            select: 'username first_name last_name picture'
        }).select("-password")
...
exports.addFeatureImage = async (req, res) => {
    try {
        ...
        if (images.length < 1) return res.status(400).send({ message: "Select photo and try agin !" })
        let uploaded = []
